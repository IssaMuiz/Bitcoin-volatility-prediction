...
- The model achieve RMSE below 30–40% of mean volatility
...
After training the time-series regression model, performance was evaluated on the validation dataset using Mean Absolute Error (MAE), Root Mean Squared Error (RMSE), and R² Score.
...
**Mean Absolute Error (MAE)** measures the average absolute difference between predicted and actual volatility values. An MAE of 0.295 indicates that the model’s predictions deviate from the true value by approximately 0.29 on average.

**Root Mean Squared Error (RMSE)** measures prediction error while giving more weight to larger mistakes.
The RMSE of 0.474 shows that although most predictions are close to actual values, the model experiences some larger deviations, which is expected in financial time-series due to sudden market movements.

**R² Score** represents the proportion of variance in the target variable explained by the model.
An R² of 0.855 means that approximately 85.5% of the variation in Bitcoin volatility is captured by the model, indicating strong explanatory power.

**Summary**
The evaluation demonstrates that the model effectively captures temporal patterns in the data using engineered time-series features such as lagged values, rolling statistics, and transformed variables. While some prediction error remains due to the inherently unpredictable nature of financial markets, the model provides a strong baseline for forecasting volatility and can be further improved through model tuning and feature refinement.
